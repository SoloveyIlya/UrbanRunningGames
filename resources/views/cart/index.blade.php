...
@section('content')
<div class="cart-page">
    <div class="cart-page__header container pt-8 md:pt-12">
        <nav class="shop-breadcrumb mb-0" aria-label="Хлебные крошки">
            ...
            <span class="shop-breadcrumb__sep" aria-hidden="true">/</span>
            <span class="shop-breadcrumb__current" aria-current="page">Корзина</span>
        </nav>

        <h1 class="shop-title">Корзина</h1>

        <p class="shop-subtitle">Проверьте состав заказа перед оформлением заявки.</p>
    </div>
            ...
            @if(count($items) > 0)
                        ...
                        <tbody>
                            @foreach($items as $row)
                                <tr class="cart-row">
                                    ...
                                    <td data-label="Вариант" class="cart-td-variant">
                                        {{ $row['variant'] ? $row['variant']->attribute_label : '—' }}
                                        @if($row['variant'] && $row['variant']->gender)
                                            <span class="cart-item-gender">Пол: {{ $row['variant']->gender_label }}</span>
                                        @endif
                                    </td>
                                    ...
                                    <td data-label="Кол-во" class="cart-td-qty">
                                        <form action="{{ route('cart.update') }}" method="POST" class="cart-qty-form">
                                            ...
                                            <input type="number" name="quantity" value="{{ $row['quantity'] }}" min="1" max="99" class="cart-qty-input" aria-label="Количество" onchange="this.form.submit()">
                                        </form>
                                    </td>
                ...
                <div class="cart-summary">
                        ...
                        @if($promo ?? null)
                            <div class="cart-promo-applied">
                                ...
                                <form action="{{ route('cart.promo.remove') }}" method="POST" class="cart-promo-remove-form">
                                    @csrf
                                    <button type="submit" class="shop-filter-btn shop-filter-btn--active">Отменить</button>
                                </form>
                            </div>
                        @else
                            <form action="{{ route('cart.promo.apply') }}" method="POST" class="cart-promo-form">
                                ...
                                <button type="submit" class="shop-filter-btn shop-filter-btn--active">Применить</button>
                            </form>
                        @endif
                    </div>
                    ...
                    <div class="cart-actions">
                        <a href="{{ route('shop.index') }}" class="shop-filter-btn">Продолжить покупки</a>
                        <a href="{{ route('checkout.show') }}" class="shop-filter-btn shop-filter-btn--active">Оформить заявку</a>
                    </div>
                </div>
            @else
                <div class="cart-empty">
                    ...
                    <a href="{{ route('shop.index') }}" class="shop-filter-btn">Перейти в каталог</a>
                </div>
            @endif
        </div>
    </div>
